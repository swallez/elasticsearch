...
public abstract class BlockConverter {
    ...
    protected final <T extends Block> T castBlock(Block block, Class<T> clazz) {
        if (clazz.isAssignableFrom(block.getClass())) {
            ...
            throw new IllegalArgumentException("Field type [" + esqlType + "] expects a [" + clazz.getSimpleName() +
                "] but we got a [" + block.getClass().getSimpleName() + "]");
        }
    }
    ...
    public static class TransformedBytesRef extends BytesRefConverter {
        ...
        private BytesRefBlock transformValues(BytesRefBlock block) {
            BlockFactory blockFactory = BlockFactory.getInstance(
                ...
                new NoopCircuitBreaker("esql-arrow"), BigArrays.NON_RECYCLING_INSTANCE
            );
