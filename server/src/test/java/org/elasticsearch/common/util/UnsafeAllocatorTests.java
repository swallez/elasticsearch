...
public class UnsafeAllocatorTests extends ESTestCase {

    private static final Logger logger = LogManager.getLogger(UnsafeAllocatorTests.class);
    ...
    public void testFastPathIsEnabledUnderTestJvm() {
        ...
        logger.info("UnsafeAllocator.isEnabled() = {}", UnsafeAllocator.isEnabled());
        assertTrue(
            ...
            UnsafeAllocator.isEnabled()
        );
    }

    public void testZeroLength() {
        byte[] buf = UnsafeAllocator.newUninitializedByteArray(0);
        assertEquals(0, buf.length);
    }

    public void testRequestedLength() {
        int size = randomIntBetween(1, 1 << 16);
        byte[] buf = UnsafeAllocator.newUninitializedByteArray(size);
    ...
    public void testIndependentAllocations() {
        ...
        int size = randomIntBetween(1, 4096);
        byte[] a = UnsafeAllocator.newUninitializedByteArray(size);
        byte[] b = UnsafeAllocator.newUninitializedByteArray(size);
        assertNotSame(a, b);
